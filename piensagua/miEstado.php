...
<head>
  ...
  <title>Piensagua | Mi Estado</title>
  ...
  <script src="JS/jquery-1.10.2.min.js"></script>
	...
	<?php
		...
		if (($handle = fopen("src/gast_agua_estados.csv", "r")) !== FALSE) {
			while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
				$csv_data[$id]['edo'] = $data[0];
				$csv_data[$id]['ing_total'] = $data[2];
				$csv_data[$id]['poblacion'] = $data[3];
				$csv_data[$id]['anual'] = $data[4];
				$csv_data[$id]['mensual'] = $data[5];
	...
	?>	
		...
		function compara_gasto() {
			var pago = $('#pago').val();
			var habitantes = $('#habitantes').val();
			var media = parseFloat(json_arr[id_edo_global]['mensual']);
			media = media.toFixed(3);
			var diferencia = media - pago/habitantes;
			diferencia = diferencia.toFixed(2);
			if(diferencia > 0.009) {
                var percent = diferencia*100/1522.46; 
				$('#myAlert').removeClass('alert-danger estado');
                $('#myAlert').removeClass('alert-info estado');
				$('#myAlert').addClass("alert-success estado");
				$('#myAlert').html('<a href="#" class="close" data-dismiss="closeAlert()">&times;</a>\
									<strong>¡Felicidades!</strong> Tu consumo está $'+Math.round(diferencia * 100) / 100 +' abajo de la media de tu estado. Podrías comprar '+percent.toFixed(2) +'% más de la canasta básica con lo que ahorras. <a href="informacion.html">Investiga como ahorrar aún más.</a>');
				$('#myAlert').css("display","block");
			}
            else if(diferencia < 0.009 && diferencia>-0.009) {
				$('#myAlert').removeClass('alert-danger estado');
                $('#myAlert').removeClass('alert-success estado');
				$('#myAlert').addClass("alert-info estado");
				$('#myAlert').html('<a href="#" class="close" data-dismiss="closeAlert()">&times;</a>\
									<strong>¡Felicidades!</strong> Tu consumo es óptimo. <a href="informacion.html">Investiga como ahorrar aún más.</a>');
				$('#myAlert').css("display","block");
			}
			else {
				diferencia *= -1;
				...
				$('#myAlert').removeClass('alert-success estado');
                $('#myAlert').removeClass('alert-info estado');
				$('#myAlert').addClass("alert-danger estado");
				$('#myAlert').html('<a href="#" class="close" onclick="closeAlert()">&times;</a>\
									<strong>¡Alerta!</strong> Tu consumo está $'+Math.round(diferencia * 100) / 100 +' arriba de la media de tu estado. Con esa cantidad comprarías un '+percent.toFixed(2) +'% más de la canasta básica. <a href="informacion.html">Investiga como ahorrar aún más.</a>');
				$('#myAlert').css("display","block");
			}
		}
		function closeAlert() {
        ...
        $(function () {
            ...
            $('.area').hover(function () {
			   ...
			   $('#ing_total').html('<b>Ingreso total por agua</b>: $'+ parseFloat(json_arr[id_estado]['ing_total']));
			   $('#pob').html('<b>Población</b>: '+ parseFloat(json_arr[id_estado]['poblacion']));
			   $('#anual').html('<b>Per capita anual</b>: $'+ parseFloat(json_arr[id_estado]['anual']).toFixed(2));
			   $('#mensual').html('<b>Per capita mensual</b>: $'+ parseFloat(json_arr[id_estado]['mensual']).toFixed(2));
                ...
                if(last_selected_id_estado!==null){
					...
					$('#ing_total').html('<b>Ingreso total por agua</b>: $'+ parseFloat(json_arr[last_selected_id_estado]['ing_total']));
					$('#pob').html('<b>Población</b>: '+parseFloat(json_arr[last_selected_id_estado]['poblacion']));
					$('#anual').html('<b>Per capita anual</b>: $'+parseFloat(json_arr[last_selected_id_estado]['anual']).toFixed(2));
					$('#mensual').html('<b>Per capita mensual</b>: $'+ parseFloat(json_arr[last_selected_id_estado]['mensual']).toFixed(2));
                }else{                    
					$('#edo').html("&nbsp;");
          ...
          <div class="row">
            <div class="col-lg-12">
              <div class="about-white-text titulosSecciones">
                <h1><i class="material-icons" style="font-size:36px">event_available</i><a href="#mapaInteractivo">Participa!</a></h1>
              </div>
            </div>
            <div class="col-lg-12">
              <div class="about-white-desc">
                <ul>
                    ...
                    <li>Ingresa el monto que pagas en tu recibo</li>
                    <li>Indica cuantas personas viven en tu hogar</li>
                    <li>Compara tus resultados y reflexiona</li>
                </ul>
              </div>
            </div>
          </div>
...
<form id="form1" >
      <div id="control_map">
        ...
        <div id='content_mapa'>
		   ...
		   <div id="ing_total"></div>
		   <div id="pob"></div>
		   <div id="anual"></div>
		   <div id="mensual"></div>
		
			<div id='survey' style="display:none">
				<p>Compara tu consumo</p>
		        <form>
					<input id="estado" name="estado" type="text" placeholder="Estado" disabled></input>
					<input id="pago" name="pago" type="text" placeholder="Pago mensual"></input>
					<input id="habitantes" name="habitantes" type="text" placeholder="Habitantes del hogar"></input>
					<button id="cmp_btn" type="button" onclick="compara_gasto()">Comparar</button> 
				</form>
			</div>
        </div>   
            ...
            <div class="row">
                <div class="col-lg-12">
                    <a href="/src/gast_agua_estados.csv" target="_blank" download="gast_agua_estados.csv"><p>Datos extraidos de INEGI 2017</p></a>
                </div>
            </div>
